...
def train_model(config):
    ...
    # ── step3: 创建模型权重保存目录 ──────────────────────────────────
    # 目录名形如 "fashion_mnist_weights"，用于存放每个 epoch 的 .pt 权重
    Path(f"{config['datasource']}_{config['model_folder']}").mkdir(parents=True, exist_ok=True)

    # ── step4: 获取数据 ──────────────────────────────────────────────
    # 返回三个 DataLoader，迭代它们时每次吐出一个 batch 的 (x, y)
    train_loader, val_loader, test_loader = get_ds(config)

    # ── step5: 构建模型并搬到设备 ────────────────────────────────────
    model = get_model(config).to(device)

    # ── step6: 打印参数量 ────────────────────────────────────────────
    # p.numel() 返回单个参数的个数；sum(...) 汇总所有参数 = 模型大小
    print(f"模型参数量: {sum(p.numel() for p in model.parameters()):,}")

    # ── step7: 创建优化器 ────────────────────────────────────────────
    # Adam 是自适应学习率的优化器；eps=1e-9 是防止除零的小常数
    optimizer = torch.optim.Adam(model.parameters(), lr=config['lr'], eps=1e-9)

    # ── step8: 创建损失函数 ──────────────────────────────────────────
    # CrossEntropyLoss 内部自带 Softmax，所以模型最后一层不要自己加激活函数
    # （model.py 里最后一层 Linear 不加激活就是这个原因）
    ...
    # ── 全部 epoch 训完，最后在测试集上做一次终极评估 ─────────────────
    # 测试集在训练中从未见过，用来衡量模型的真实泛化能力
